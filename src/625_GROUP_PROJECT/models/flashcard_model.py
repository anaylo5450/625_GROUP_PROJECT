...
def get_card_by_id(card_id, deck_id):
    conn = get_db()
    card = conn.execute(
        'SELECT * FROM flashcards WHERE id = ? AND deck_id = ?', (card_id, deck_id)
    ).fetchone()
    conn.close()
    return card
...
def update_card(card_id, deck_id, front, back):
    conn = get_db()
    conn.execute(
        'UPDATE flashcards SET front=?, back=?, updated_at=CURRENT_TIMESTAMP WHERE id=? AND deck_id=?',
